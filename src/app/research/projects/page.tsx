...
const Projects: FC = () => {
  ...
  useEffect(() => {
    fetch('/data/research.json')
      .then((res) => res.json())
      .then((data) => {
        const patentItems = data.filter((item: Blogs) => item.type === 'project');
        setBlogs(patentItems);

        // Extract unique keywords from blogs
        const allKeywords = new Set<string>();
        patentItems.forEach((item: Blogs) => {
          item.keywords.forEach((keyword: string) => allKeywords.add(keyword));
        });
        setKeywords(Array.from(allKeywords));
        setFilteredBlogs(patentItems);
      })
    ...
    <div className="container mx-auto p-2 flex">
      ...
      <div className="w-3/4 p-4">
        <p className="text-sm p-4 pt-0 pb-2">
          ...
          &nbsp;&gt;&gt;&nbsp;
          <a href="/research/patents" className="hover:underline">
            Patents
          </a>
        </p>
